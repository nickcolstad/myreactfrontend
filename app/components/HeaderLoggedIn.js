...
function HeaderLoggedIn(props) {
  ...
  function handleLogout() {
    appDisptach({ type: "logout" });
    appDisptach({ type: "flashMessage", value: "You have successfully logged out" });
  }

  function handleSearchIcon(e) {
    e.preventDefault();
    appDisptach({ type: "openSearch" });
  }
  ...
  return (
    <div className="flex-row my-3 my-md-0">
      ...
      <span onClick={() => appDisptach({ type: "toggleChat" })} data-for="chat" data-tip="Chat" className={"mr-2 header-chat-icon " + (appState.unreadChatCount ? "text-danger" : "text-white")}>
